...
class MultiInterestExtractor(torch.nn.Module):
    def __init__(self, hidden_units, num_interests, dropout_rate=0.1):
        ...
        self.attention = torch.nn.Sequential(
            ...
            torch.nn.Linear(hidden_units, num_interests, bias=False)
        )
        self.layernorm = torch.nn.LayerNorm(hidden_units)
    ...
    def forward(self, seqs, mask):
        ...
        interests = torch.einsum("btd,btk->bkd", seqs, attn_weight)
        return interests
...
class BaselineModel(torch.nn.Module):
    ...
    def __init__(self, user_num, item_num, feat_statistics, feat_types, args):  #
        ...
        # self.interest_extractor = MultiInterestExtractor(args.hidden_units, args.num_interests, args.dropout_rate)

    def get_l2_loss(self, l2_lambda=1e-6):
        l2_loss = 0.0
        for emb in self.sparse_emb.values():
            l2_loss += torch.norm(emb.weight, 2)
        l2_loss += torch.norm(self.item_emb.weight, 2)
        l2_loss += torch.norm(self.user_emb.weight, 2)
    ...
    def forward(
        ...
        # final_interest = interests.mean(dim=1)
    ...
    def predict(self, log_seqs, seq_feature, mask):
        ...
        # final_feat = interests.mean(dim=1)
